...
#region agent log
DEBUG_LOG="$WORKSPACE/.cursor/debug-395fbc.log"
mkdir -p "$(dirname "$DEBUG_LOG")"
python -c "import json,time; p='$DEBUG_LOG'; open(p,'a',encoding='utf-8').write(json.dumps({'sessionId':'395fbc','runId':'pre-fix','hypothesisId':'H3_H5','location':'train_tf.sh:before_nbconvert','message':'Starting TF nbconvert execution','data':{'tf_notebook':'$TF_NOTEBOOK','exec_nb':'$exec_nb','cuda_visible_devices':'$TF_GPUS','conda_env':'$CONDA_ENV'},'timestamp':int(time.time()*1000)}) + '\n')"
#endregion
jupyter nbconvert --to notebook --execute --output "$exec_nb" --ExecutePreprocessor.timeout=-1 "$TF_NOTEBOOK" 2>&1 | tee "$log"
exit_code="${PIPESTATUS[0]}"
#region agent log
python -c "import json,time,os; p='$DEBUG_LOG'; open(p,'a',encoding='utf-8').write(json.dumps({'sessionId':'395fbc','runId':'pre-fix','hypothesisId':'H3_H5','location':'train_tf.sh:after_nbconvert','message':'Finished TF nbconvert execution','data':{'exit_code':$exit_code,'exec_nb_exists':os.path.isfile('$exec_nb'),'log_path':'$log'},'timestamp':int(time.time()*1000)}) + '\n')"
#endregion
